...
// A line is flagged unavailable when a row exists in unavailable_lines for that
// (order, sku). Stored in Supabase so the marks survive a refresh.
// Returns a Map<orderId, Set<sku>> for a batch of orders.
export async function unavailableMap(orderIds) {
  if (!orderIds.length) return new Map();
  const list = orderIds.map((id) => `"${id}"`).join(",");
  const rows = (await supa(`unavailable_lines?maropost_order_id=in.(${list})&select=maropost_order_id,sku`)) || [];
  const m = new Map();
  for (const r of rows) {
    const id = String(r.maropost_order_id);
    if (!m.has(id)) m.set(id, new Set());
    m.get(id).add(String(r.sku));
  }
  return m;
}

export async function unavailableSet(orderId) {
  return (await unavailableMap([String(orderId)])).get(String(orderId)) || new Set();
}

// Flag (or unflag) one line as not available.
export async function setUnavailable(orderId, sku, flag) {
  if (flag) {
    return supa("unavailable_lines", {
      method: "POST",
      headers: { Prefer: "resolution=merge-duplicates" },
      body: JSON.stringify({ maropost_order_id: String(orderId), sku: String(sku) }),
    });
  }
  return supa(
...
export function reviewOrder(order, itemMap, unavailable = new Set()) {
  ...
  const reviewed = lines.map((l) => {
    ...
    return {
      ...
      unavailable: unavailable.has(sku),
    };
  });
  // Value of the not-available lines = what we should refund (GST-inclusive,
  // since the prices already include GST).
  const refundTotal = Number(reviewed.filter((l) => l.unavailable).reduce((s, l) => s + l.line_total, 0).toFixed(2));
...
// Find the Stripe payment for a Maropost order by searching PaymentIntents
// (then Charges) for the order id in metadata. The metadata key can vary by
// integration, so we try a configurable key first then common fallbacks.
// Returns { paymentIntentId, chargeId, amount, currency, refunded } or null.
export async function findStripePayment(orderId) {
  const raw = String(orderId);
  const digits = raw.replace(/\D/g, ""); // "27575" from "N27575"
  // Maropost ids carry an "N" prefix (N27575) but Stripe metadata often stores
  // just the number (27575) - try both forms.
  const ids = [...new Set([raw, digits].filter(Boolean))];
  const keys = [...new Set([
    ...(process.env.STRIPE_ORDER_ID_META_KEY ? [process.env.STRIPE_ORDER_ID_META_KEY] : []),
    "order_id", "OrderID", "order_number", "orderId", "neto_order_id", "maropost_order_id", "order", "OrderNumber",
  ])];
  for (const k of keys) {
    for (const idv of ids) {
      const q = encodeURIComponent(`metadata['${k}']:'${idv}'`);
      // PaymentIntents first (metadata usually lives here), then Charges.
      for (const kind of ["payment_intents", "charges"]) {
        let data;
        ...
        if (kind === "payment_intents") {
          return { paymentIntentId: hit.id, chargeId: hit.latest_charge || null, amount: hit.amount, currency: hit.currency, refunded: 0, matchedKey: k, matchedId: idv };
        }
        return { paymentIntentId: hit.payment_intent || null, chargeId: hit.id, amount: hit.amount, currency: hit.currency, refunded: hit.amount_refunded || 0, matchedKey: k, matchedId: idv };
      }
    }
  }
  return null;
