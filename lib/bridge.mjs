...
//   2. Search PaymentIntents, Charges, then Checkout Sessions by the order id in
//      metadata (trying a configurable key + common fallbacks, raw + digits).
//   3. Last resort: scan the most recent PaymentIntents/Charges and match the
//      order number anywhere in metadata values or the description.
// Returns { paymentIntentId, chargeId, amount, currency, refunded, matchedBy } or null.
export async function findStripePayment(orderId, explicitId = null) {
  if (explicitId) {
  ...
  const keys = [...new Set([
    ...
    "order", "OrderNumber", "OrderId", "neto_order", "ref", "reference", "invoice",
  ])];
  ...
  for (const k of keys) {
    for (const idv of ids) {
      ...
      for (const kind of ["payment_intents", "charges", "checkout/sessions"]) {
        ...
        if (!hit) continue;
        if (kind === "payment_intents") return shapePI(hit, `metadata:${k}`);
        if (kind === "charges") return shapeCharge(hit, `metadata:${k}`);
        // checkout session -> resolve its PaymentIntent
        if (hit.payment_intent) {
          try { return shapePI(await stripeGet(`/payment_intents/${hit.payment_intent}`), `session.metadata:${k}`); } catch { /* */ }
        }
      }
    }
  }

  // 3. Brute-force fallback: scan recent payments and match the order number in
  //    metadata values OR the description. The Neto/Stripe plugin writes the id
  //    only into the description (e.g. "Charge for N27575 (...)"), never into a
  //    queryable metadata key, so this is the path that actually finds it.
  ...
  const matches = (obj) => {
    ...
    return !!(digitRe && digitRe.test(desc));
  };
  ...
  for (const kind of ["payment_intents", "charges"]) {
    ...
    for (let page = 0; page < 5; page++) {
      ...
      if (hit) return kind === "payment_intents" ? shapePI(hit, "scan-desc") : shapeCharge(hit, "scan-desc");
