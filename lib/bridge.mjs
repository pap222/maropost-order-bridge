...
export function buildPayload(order, itemMap) {
  ...
  // QuickB2B's delivery-date field drives its scheduling, so it must carry the
  // date the CUSTOMER wants the order (DateRequired) - not today's push date -
  // otherwise QuickB2B schedules it onto the next delivery run. Formatted
  // YYYY-MM-DD in Wagga time (same source as the dashboard's "wanted" date) so it
  // never lands a day early.
