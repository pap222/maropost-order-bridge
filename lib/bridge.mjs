...
export async function findStripePayment(orderId, explicitId = null) {
  ...
  // 3. Brute-force fallback: scan recent payments for the order number anywhere.
  ...
  const matches = (obj) => {
    ...
    for (const v of Object.values(meta)) {
      ...
      if (idSet.has(vs)) return true;
    }
    const desc = String(obj.description || "").toLowerCase();
    return [...idSet].some((id) => id && desc.includes(id));
  };
  for (const kind of ["payment_intents", "charges"]) {
    let data;
    try { data = await stripeGet(`/${kind}?limit=100`); } catch { continue; }
    const hit = (data.data || []).find(matches);
