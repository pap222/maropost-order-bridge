...
export async function findStripePayment(orderId) {
  ...
  const keys = [
    ...
    "order_id", "OrderID", "order_number", "orderId", "neto_order_id", "maropost_order_id",
  ];
  const tried = new Set();
  for (const k of keys) {
    if (tried.has(k)) continue;
    tried.add(k);
    const q = encodeURIComponent(`metadata['${k}']:'${id}'`);
    ...
    for (const kind of ["payment_intents", "charges"]) {
      ...
      if (kind === "payment_intents") {
        return {
          paymentIntentId: hit.id,
          chargeId: hit.latest_charge || null,
          amount: hit.amount,
      ...
      return {
        paymentIntentId: hit.payment_intent || null,
        chargeId: hit.id,
        amount: hit.amount,
        currency: hit.currency,
        refunded: hit.amount_refunded || 0,
      };
    }
  }
  return null;
