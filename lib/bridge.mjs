...
const ORDER_OUTPUT = [
  ...
  // The ORDER's own ship/bill address MUST be pulled via the "ShipAddress" /
  // "BillAddress" GROUP tokens. Requesting the flat fields (ShipStreetLine1, …)
  // individually returns them EMPTY for web orders, which used to make us fall
  // back to the customer account's default address — often stale/wrong (e.g. a
  // customer whose default ship is an old address delivered to the wrong place).
  // The group tokens expand to the flat Ship*/Bill* fields with the order's real
  // values, so keep both listed.
