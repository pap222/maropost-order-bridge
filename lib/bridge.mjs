...
const ORDER_OUTPUT = [
  ...
  // SKU/Quantity/OrderLineID (no ProductName).
  "OrderLine.SKU", "OrderLine.ProductName", "OrderLine.Quantity",
];
...
// comment (never sent as a raw SKU). Lines mapped to a generic MISC item get
// their real description added to the comment. Returns { payload, unmapped }.
export function buildPayload(order, itemMap) {
  ...
  for (const l of lines) {
    ...
    OrderDetail.push({ item_code: entry.code, quantity: qty });
    // Generic MISC line: the docket would just read "MISCELLANEOUS ITEM xN", so
    // record what it actually is for the packer.
    if (isMiscCode(entry.code)) miscNotes.push(`${l.ProductName || sku} x${qty}`);
  }
  const ci = customerInfo(order);
  const reqd = fmtReqDate(order.DateRequired);
  ...
  // Lead with the requested date + pickup/delivery so packing staff can't miss it.
  let comment = "";
  if (reqd) comment += `WANTS: ${reqd}`;
  if (isPickup) {
    // Pickup: shout PICK UP, no address needed.
    comment += (comment ? " | " : "") + "PICK UP";
  } else {
    // Delivery: flag it and put the address right up front.
    comment += (comment ? " | " : "") + "DELIVERY";
    if (ci.address) comment += ` | ${ci.address}`;
  }
  comment += (comment ? " | " : "") + ci.name;
  // Include the business/company name when present (deliveries often go to a
  // business, e.g. a childcare centre) and it isn't already the displayed name.
  if (ci.company && ci.company !== ci.name) comment += ` | ${ci.company}`;
  // Always include the email (it's the reliable customer identifier here),
  // unless the name already IS the email.
  if (ci.email && ci.email !== ci.name) comment += ` | ${ci.email}`;
  if (ci.phone) comment += ` | ${ci.phone}`;
  comment += ` | WEB #${order.OrderID}`;
  // Spell out what each generic MISC line actually is so packers know what to pack.
  if (miscNotes.length) comment += ` | MISC ITEMS: ${miscNotes.join("; ")}`;
